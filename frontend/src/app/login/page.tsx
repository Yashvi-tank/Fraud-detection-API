...
import { ShieldCheck, LogIn, Lock, User } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";

export default function LoginPage() {
  const router = useRouter();
  const { login, loginAsDemo, token, isAuthenticated, isLoading, error, clearError } = useAuth();
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-6 py-12 relative font-sans">
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="font-heading text-2xl font-bold tracking-tight text-slate-100">Welcome Back</h2>
          <p className="text-sm text-slate-400 mt-1">Sign in to manage transaction risk analytics</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-900 bg-slate-950/60 backdrop-blur-md p-8 shadow-2xl">
          ...
          <div className="relative flex py-5 items-center">
            ...
            <span className="flex-shrink mx-4 text-[10px] uppercase tracking-wider font-semibold text-slate-500">Or</span>
            <div className="flex-grow border-t border-slate-900"></div>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full rounded-lg bg-slate-900 py-3 text-sm font-semibold text-slate-200 border border-slate-800 hover:bg-slate-800 transition-colors"
          >
            Quick Access: Try Demo Mode
          </button>
        </div>
